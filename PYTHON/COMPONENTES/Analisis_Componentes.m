...
%   CON_RESISTENCIAS
%
% Cada archivo se trata como una condicion distinta.
% Calcula el indice de compromiso estabilidad-deteccion.
%
% La grafica permite mover puntos con el raton:
%   click sobre un punto -> arrastrar verticalmente -> soltar.
%
% Se conserva:
...
carpeta = "D:\TFM\PYTHON\COMPONENTES";
...
fprintf("\n============================================================\n");
fprintf("ANALISIS CON/SIN RESISTENCIAS\n");
fprintf("Carpeta: %s\n", char(carpeta));
fprintf("Primeras muestras descartadas: %d\n", numMuestrasInicialesDescartar);
fprintf("Indice: %s\n", char(tipoIndice));
fprintf("============================================================\n");

Datos = leerCarpetaComponentes(carpeta);

if isempty(Datos)
    error("No se han leido datos. Revisa la carpeta y los archivos.");
end
...
Datos = sortrows(Datos, {'CondicionComponentes','EstadoSonda','ConfigID','Idx'});

ConfigInfo = crearTablaConfiguraciones(ventanasOrden, etapasOrden);

fprintf("\nCondiciones leidas:\n");
disp(unique(Datos(:, {'CondicionComponentes','PCB_Original'}), 'rows'));
...
fprintf("\n============================================================\n");
fprintf("TABLA FINAL: COMPROMISO CON/SIN RESISTENCIAS\n");
fprintf("============================================================\n");
...
assignin('base', 'DatosComponentes', Datos);
assignin('base', 'ResumenCompromisoComponentes', ResumenFinal);
assignin('base', 'RankingCompromisoComponentes', RankingCompromiso);
assignin('base', 'ConfigInfoComponentes', ConfigInfo);

fprintf("\n============================================================\n");
fprintf("PROCESO COMPLETADO\n");
fprintf("Variables en workspace:\n");
fprintf("  DatosComponentes\n");
fprintf("  ResumenCompromisoComponentes\n");
fprintf("  RankingCompromisoComponentes\n");
fprintf("  ConfigInfoComponentes\n");
fprintf("\nCuando muevas puntos en la grafica se exportara tambien:\n");
fprintf("  ResumenCompromisoComponentes_Ajustado\n");
fprintf("  RankingCompromisoComponentes_Ajustado\n");
fprintf("============================================================\n");
...
condiciones = unique(Datos.CondicionComponentes, 'stable');

CondicionComponentes = strings(0,1);
...
for ip = 1:numel(condiciones)

    condicion = condiciones(ip);
    ...
    for v = 1:numel(ventanasOrden)
        for e = 1:numel(etapasOrden)
            ...
            Tcond = Datos( ...
                Datos.CondicionComponentes == condicion & ...
            ...
            CondicionComponentes(end+1,1) = condicion;
            PCB_Original(end+1,1) = Tcond.PCB_Original(1);

            Ventana(end+1,1) = ventana;
            Etapas(end+1,1) = etapas;
            ConfigID(end+1,1) = calcularConfigID(ventana, etapas, ventanasOrden, etapasOrden);
            ConfigLabel(end+1,1) = ventana + "-" + string(etapas) + "e";

            DispersionSinSonda_pct(end+1,1) = dispersionCond;
            CambioDirectoMedioAbs_pct(end+1,1) = cambioDirecto;
            CambioAdyacenteMedioAbs_pct(end+1,1) = cambioAdy;
            AdjSobreDirecto_pct(end+1,1) = 100 * cambioAdy / max(cambioDirecto, eps);
...
Resumen = sortrows(Resumen, {'CondicionComponentes','ConfigID'});
...
function Datos = leerCarpetaComponentes(carpeta)
...
for i = 1:numel(d)
    ...
    [~, nombreBase, ~] = fileparts(char(nombre));
    condicion = string(nombreBase);
    ...
    T.Archivo = repmat(condicion, n, 1);
    T.CondicionComponentes = repmat(condicion, n, 1);
...
for v = 1:numel(ventanasOrden)
    for e = 1:numel(etapasOrden)
        ...
        ConfigID(end+1,1) = contador;
        Ventana(end+1,1) = ventanasOrden(v);
        Etapas(end+1,1) = etapasOrden(e);
        ConfigLabel(end+1,1) = ventanasOrden(v) + "-" + string(etapasOrden(e)) + "e";
...
fig = figure( ...
    'Name', 'Compromiso estabilidad-deteccion con/sin resistencias', ...
    'NumberTitle', 'off');
...
condiciones = unique(Resumen.CondicionComponentes, 'stable');
colores = lines(numel(condiciones));

if usarOffset && numel(condiciones) > 1
    offsets = linspace(-ampOffset, ampOffset, numel(condiciones));
else
    offsets = zeros(1,numel(condiciones));
end
...
for i = 1:numel(condiciones)
    ...
    filas = find(Resumen.CondicionComponentes == condicion);
...
title('Comparacion del indice de compromiso con y sin resistencias', 'Interpreter', 'none');
...
annotation( ...
    ...
    'textbox', ...
    [0.13 0.01 0.75 0.04], ...
    'String', 'Click sobre un punto y arrastra verticalmente para modificarlo.', ...
    'EdgeColor', 'none', ...
...
assignin('base', 'ResumenCompromisoComponentes_Ajustado', ResumenAjustado);
assignin('base', 'RankingCompromisoComponentes_Ajustado', RankingAjustado);

fprintf("\nPunto actualizado:\n");
fprintf("  Condicion    : %s\n", char(ResumenAjustado.CondicionComponentes(rowIndex)));
