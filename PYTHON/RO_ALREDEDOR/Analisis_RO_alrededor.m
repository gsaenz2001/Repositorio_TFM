function Analisis_RO_alrededor()
% ANALISIS RO ALREDEDOR
%
% Compara dos archivos de medida:
%   SIN_RO
%   CON_RO
%
% Cada archivo se trata como una condicion distinta:
%   - SIN_RO: medida sin osciladores de anillo alrededor
%   - CON_RO: medida con osciladores de anillo alrededor
%
% Calcula el mismo analisis de compromiso:
%   - dispersion sin sonda
%   - cambio directo medio con sonda
%   - cambio adyacente medio con sonda
%   - indice de compromiso estabilidad-deteccion
%
% La grafica permite mover manualmente los puntos:
...
carpeta = "D:\TFM\PYTHON\RO_ALREDEDOR";

nombreAnalisis = "RO_ALREDEDOR";

ventanasOrden = ["100ms", "10ms", "1ms", "100us"];
etapasOrden   = [3, 5, 51, 101];

% Condicion descartada.
...
% Usar todas las muestras restantes.
...
% Tipo de indice:
%   "geometrico": sqrt(EstabilidadScore * DeteccionScore)
%   "media": media aritmetica 50/50
tipoIndice = "geometrico";

% Separacion horizontal entre condiciones.
...
fprintf("\n============================================================\n");
fprintf("ANALISIS DE COMPROMISO CON/SIN RO ALREDEDOR\n");
fprintf("Analisis: %s\n", char(nombreAnalisis));
fprintf("Carpeta: %s\n", char(carpeta));
...
Datos = sortrows(Datos, {'CondicionRO','EstadoSonda','ConfigID','Idx'});

ConfigInfo = crearTablaConfiguraciones(ventanasOrden, etapasOrden);

fprintf("\nCondiciones leidas:\n");
disp(unique(Datos(:, {'CondicionRO','PCB_Original'}), 'rows'));
...
fprintf("\n============================================================\n");
fprintf("TABLA FINAL: COMPROMISO ESTABILIDAD-DETECCION CON/SIN RO ALREDEDOR\n");
fprintf("============================================================\n");
...
assignin('base', 'DatosRO', Datos);
assignin('base', 'ResumenCompromisoRO', ResumenFinal);
assignin('base', 'RankingCompromisoRO', RankingCompromiso);
assignin('base', 'ConfigInfoRO', ConfigInfo);

fprintf("\n============================================================\n");
fprintf("PROCESO COMPLETADO\n");
fprintf("Variables en workspace:\n");
fprintf("  DatosRO\n");
fprintf("  ResumenCompromisoRO\n");
fprintf("  RankingCompromisoRO\n");
fprintf("  ConfigInfoRO\n");
fprintf("\nCuando muevas puntos en la grafica se exportara tambien:\n");
fprintf("  ResumenCompromisoRO_Ajustado\n");
fprintf("  RankingCompromisoRO_Ajustado\n");
fprintf("============================================================\n");
...
condiciones = unique(Datos.CondicionRO, 'stable');

CondicionRO = strings(0,1);
...
for ip = 1:numel(condiciones)
    ...
    for v = 1:numel(ventanasOrden)
        for e = 1:numel(etapasOrden)
            ...
            CondicionRO(end+1,1) = condicion; %#ok<AGROW>
...
Resumen = sortrows(Resumen, {'CondicionRO','ConfigID'});
...
%% ========================================================================
%  LECTURA DE ARCHIVOS RO
%% ========================================================================

function Datos = leerCarpetaRO(carpeta)
...
for i = 1:numel(d)
    ...
    T.CondicionRO = repmat(condicion, n, 1);
...
fig = figure( ...
    'Name', 'Compromiso estabilidad-deteccion con/sin RO alrededor', ...
    'NumberTitle', 'off');
...
condiciones = unique(Resumen.CondicionRO, 'stable');
...
for i = 1:numel(condiciones)
    ...
    filas = find(Resumen.CondicionRO == condicion);
...
title('Comparacion del indice de compromiso con y sin RO alrededor', 'Interpreter', 'none');
...
assignin('base', 'ResumenCompromisoRO_Ajustado', ResumenAjustado);
assignin('base', 'RankingCompromisoRO_Ajustado', RankingAjustado);

fprintf("\nPunto actualizado:\n");
fprintf("  Condicion RO : %s\n", char(ResumenAjustado.CondicionRO(rowIndex)));
